...
function stripRollupError(stdout) {
  return stdout.replace(/Error: Entry module cannot be external[^]+/, ''); // this is OK to show to the user but annoying to have in a test
}
        ...
        stripWhitespace(
          stripConfigErrorPath(
            stripResolveErrorPath(
              stripBenchmark(
                stripChunkHash(
                  stripStats(stripAnsiEscapes(stripRollupError(stripNodeBuiltIn(all)))),
                ),
              ),
            ),
          ),
